...
#print(datetime.strftime('%d/%m/%Y_%H:%M'))
######################## SAC
...
def price(saldo,prazo,taxa,mes=0,total=0,juros=0):
    ...
    for t in range(prazo):
        ...
        ar.write("\n%d\t%0.2f\t%0.2f\t%0.2f\t%0.2f\t%0.2f" %(mes ,saldo_final,prestacao, amortizacao,juros,saldo))
        print ("%d ->    %0.2f    %0.2f    %0.2f    %0.2f    %0.2f" %(mes,saldo_final , prestacao, amortizacao, juros , saldo  ))
    print("\n \t \t  O JUROS TOTAL É ->   %0.2f "%(total))
    ar.write("\n \t \t  O JUROS TOTAL É ->   %0.2f "%(total))
...
while executar:
    ...
    ar_time=str(strftime('%d-%m-%Y_%H_%M'))
    ar_name="relatorio_"+ar_time[:]+".xlsx"
    ar=open(ar_name[:],'w')
    ...
    while saldo_vazio:
        ...
        if saldo=="":
            ...
            print("Saldo negativo ou igual a zero")
        else:
            saldo_vazio=False
            saldo= float(saldo)
...
##############
    ar.close()
##############
    print("\tO RELATORIO ",ar_name," COM A SIMULAÇÃO DO FINANCIAMENTO FOI CRIADO \n")

    validacao_vazio=True
    while validacao_vazio:
        simulacao=input("\n\nDeseja fazer outra simulação?(s/n)>")
        if simulacao.upper() =="S":
            print(" ")
            validacao_vazio=False
        elif simulacao.upper() =="N":
            executar=False
            validacao_vazio=False
            print("")
        else:
